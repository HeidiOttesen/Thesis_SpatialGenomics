...
7. Once you have this list, you will combine all the counts associated to each group.
...
## Create groups of barcodes (from knn)
...
There are also 14 leftovers no in any group If I understand correctly?
```{r}

knn5 <- knn50[1:l]

bc <- barcodes
knn5.bc <- vector(mode="list", length=l)
i <- 1
...
while(i <= l){
  ...
  while(j <= k){
    ix <- knn5[[i]][j]
    knn5.bc[[i]][j] <- barcodes[ix]
    j <- j + 1
    bc <- bc[!bc %in% barcodes[ix]]
  }
  i <- i + 1
}
...
while(length(bc) >0){
  ...
  pool <- bc[i] #select a barcode from top to bottom
  tmp[i] <- cbind(pool)
  inter <- pool
  converge <- FALSE
  ...
  while(!converge){
    ...
    if(length(inter)==1){
      ...
      #------------------------------------------------------------#
      ls[i] <- pool
      bc <- bc[!bc %in% pool] #remove used barcode
      i <- i + 1
      converge <- TRUE
    }
  }
