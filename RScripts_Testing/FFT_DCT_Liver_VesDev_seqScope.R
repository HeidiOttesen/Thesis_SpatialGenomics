...
## Heidi Adding stuff
ss1 <- subset(image, select=c(x, y, cc, value)) %>% filter(x > 800 & x < 1700)
ss.eq <- subset(im.eq, select=c(x, y, cc, value)) %>% filter(x > 800 & x < 1700) 
ss.reg <- subset(im.reg, select=c(x, y, cc, value)) %>% filter(x > 800 & x < 1700) 
ss.itSeg <- subset(im.itSeg, select=c(x, y, cc, value)) %>% filter(x > 800 & x < 1700) 




#### DCT ImagerExtra
pdf("DCT_LiverSeqScope_1slide_comb.pdf", width = 6, height= 5)
layout(matrix(c(1,3,2,4), 2, 2))
...
plotTmp <- im.terr %>% filter(x > 800 & x < 1700)
