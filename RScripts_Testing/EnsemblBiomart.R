...
## For mouse (mm10, GRCm38):
# Don't know if there is a difference between useEnsembl and useMart?!

mm38.ensembl <- useEnsembl(dataset="mmusculus_gene_ensembl", biomart="genes", host="nov2020.archive.ensembl.org")
mm38.mart <- useMart(biomart="ENSEMBL_MART_ENSEMBL", host="nov2020.archive.ensembl.org", dataset="mmusculus_gene_ensembl", path="/biomart/martservice")


## For human (h19, GRCh37):
h37.mart <- useMart(biomart="ENSEMBL_MART_ENSEMBL", host="grch37.ensembl.org", path="/biomart/martservice", dataset="hsapiens_gene_ensembl")
h37.ensembl <- useEnsembl(dataset="hsapiens_gene_ensembl", biomart="genes", GRCh="37")
...
for(i in seq_along(fw)){
  loc <- fw
  g <- getBM(c("external_gene_name", "description", "ensembl_gene_id", "gene_biotype", "chromosome_name", "start_position","end_position", "strand"), 
                   filters=c("chromosome_name","start","end"),
                   values = pos, mart=mm38.mart)
  m.genes <- rbind(m.genes, g)
}



#BM <- getBM(c("external_gene_name", "genedb", "description", "ensembl_gene_id", "go_id", "gene_biotype", "chromosome_name", "start_position","end_position", "strand"), 
            #filters=c("chromosome_name","start","end", "go"), values = list(2, 2000,600000, TRUE), mart=mart)
searchAttributes(mart = mart, pattern = "go")
searchFilters(mart = mart, pattern = "start")

genes <- getBM(c("ensembl_gene_id","chromosome_name","start_position","end_position","strand"), 
               filters =c("chromosome_name","start"),
              values=list(c(1:22,"X","Y"),TRUE), mart=grch37)
genes <- genes[order(genes$start_position),]
genes <- genes[order(genes$chromosome_name),]



annot<-getBM(c("ensembl_gene_id","chromosome_name","start_position", "end_position"), mart=ensembl)




ensembl <- useEnsembl(biomart = "genes", dataset = "hsapiens_gene_ensembl")
ensembl <- useEnsembl(dataset="mmusculus_gene_ensembl", biomart="genes", host="jul2012.archive.ensembl.org")
