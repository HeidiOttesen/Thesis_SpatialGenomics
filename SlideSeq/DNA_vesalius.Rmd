...
sparse <- filterSparse(sparse, function(x) x <= 6)
df <- as.matrix(sparse) 
hist(sparse@x)
hist(df)
...
filter out counts above 6 
Mouse liver before: Dimensions: 2,738(bins) x 38,580(barcodes). (8,771,833 entries, 8.30% full) - remove 15.017 values

filter out columns (barcodes) with fewer than 100 non-zero values
Mouse liver after: Dimensions: 2,596 x 38,580. (8,769,416 entries, 8.76% full) - removed 142 columns
...
```{r}
setwd(in.path)
fname <- paste(format(Sys.time(), "%Y%m%d_%H%M%S"), alias, norm, dimred, "images.pdf", sep="_")
pdf(fname)
