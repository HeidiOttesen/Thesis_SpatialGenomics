...
im.path <- "~/Thesis_SpatialGenomics/SlideSeq/"
...
```{r}
fname <- paste(format(Sys.time(), "%Y%m%d_%H%M%S"), alias, "VesDev", norm, dimred, "images.pdf", sep="_")
pdf(fname)
layout(matrix(c(1,3,2,4), 2, 2))
...
file.rename(from = paste0(im.path, fname), to = paste0(out.path, fname))
```
