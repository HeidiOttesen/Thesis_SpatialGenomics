...
```{r}
fname <- "Object_DNA_Territories_mouse_liver_met_2_ChrSummed_counts0-6_logNorm_PCA10_knnDuplicate5_dims1-10_colD6_Iter10"
...
padj <- slice_min(mark, order_by=mark$p.value.adj, n=100)
int.bins <- padj
...
```{r}
logfc.min <- slice_min(padj, order_by=padj$logFC, n=10)
logfc.max <- slice_max(padj, order_by=padj$logFC, n=10)
...
for(i in seq(int.bins$genes)){
  f <- paste0(path, "BinCountLayer/", int.bins$genes[[i]], "_", vs, "_", fname, ".png")
  png(f) 
