...
ExtractMarkers - DGE between selected territories
...
vesMark <- extractMarkers(vesTerr, method=st.meth, pval=0.05)

vesMorph <- territoryMorphing(vesMark, c(5,6))

#vesTerrLay <- layerTerritory(vesMorph, c(28,34,32),morphologyFactor=0,layerDepth=5)

param <- paste(param, paste0("dims1-", PCs), paste0("colD", colD), paste0("Iter", smIter), paste0("markerMethod", st.meth), sep="_")
print(paste("Runtime so far: ", difftime(Sys.time(), start.time, units="mins"), "min"))
```
...
padj <- slice_min(m.df, order_by=m.df$p.value.adj, prop=0.05)
#logfc.min <- slice_min(m.df, order_by=m.df$logFC, prop=0.1)
#logfc.max <- slice_max(m.df, order_by=m.df$logFC, prop=0.1)
logfc.min <- slice_min(padj, order_by=padj$logFC, prop=0.05)
logfc.max <- slice_max(padj, order_by=padj$logFC, prop=0.05)
...
```{r}
#setwd(in.path)
#fname <- paste(format(Sys.time(), "%Y%m%d_%H%M%S"), "MarkerBins", alias, param, ".pdf", sep="_")
#pdf(fname)
#layout(matrix(c(1,3,2,4), 2, 2))
...
Finding marker bins between chosen territories
...
ves <- extractMarkers(vesTerr,seed=s, query=q)
