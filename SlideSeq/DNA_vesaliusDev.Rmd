...
```{r}
setwd(in.path)
fname <- paste(format(Sys.time(), "%Y%m%d_%H%M%S"), alias, param, "VesDev_imageprocessing.pdf", sep="_")
pdf(fname)
layout(matrix(c(1,3,2,4), 2, 2))
...
logfc.min <- slice_min(padj, order_by=padj$logFC, prop=0.03)
logfc.max <- slice_max(padj, order_by=padj$logFC, prop=0.03)
logfc <- bind_rows(list(logfc.min, logfc.max))
...
```{r}
layout(matrix(c(1,3,2,4), 2, 2))
for(i in seq_along(logfc)){
  print(viewGeneExpression(vesMorph, gene=i, as.layer=F,norm=F))
}
