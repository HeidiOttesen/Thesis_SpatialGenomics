...
smIter <- 15                # Smoothing Iterations
...
>>>>>>> cb544d0f261ec07cb6ca044e989c71746e3756fb
library(ggplot2)
library(patchwork)
library(viridis)
...
if(grepl("human",alias)){
  bin.all <- read.table("~/Thesis_SpatialGenomics/SlideSeq/GroupSlideSeq/hg19_1Mb_bins.txt", header = T)
  bins <- bin.all$bin_ind
} else if(grepl("mouse",alias)){
  bin.all <- read.table("~/Thesis_SpatialGenomics/SlideSeq/GroupSlideSeq/mm10_1Mb_bins.txt", header = T)
  bins <- bin.all$bin_ind
}
...
vs <- "CloneAVsNormal"
s <- c(8,9,4,6)                   #eg c(8,7) or NULL c(8,9,4,6) c(10,11)
q <- c(1,2,3,5,7,12,13)           #eg c(7,5) or NULL
pval <- 0.5
...
for(i in seq(int.bins$genes)){
  p[[i]] <- viewGeneExpression(vesMark, gene=int.bins$genes[[i]], as.layer=layer,norm=F, cex=0.1, territory1=s, territory2=q)
}

setwd(in.path)
fname <- paste(format(Sys.time(), "%Y%m%d_%H%M%S"), "MarkerBins", vs, alias, param, paste0("layer", layer), ".pdf", sep="_")
pdf(fname)
grid.arrange(p[[1]], nrow = 2)
