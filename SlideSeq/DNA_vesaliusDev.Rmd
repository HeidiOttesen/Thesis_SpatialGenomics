...
setwd(in.path)
fname <- paste(format(Sys.time(), "%Y%m%d_%H%M%S"), "Embeddings", alias, param, sep="_")
pdf(fname)
...
ExtractMarkers - DGE between selected territories
  Takes like 15mins - if not specifying seed and query..
...
vesMark <- extractMarkers(vesTerr, method=st.meth, pval=0.005)
...
```{r}
setwd(in.path)
fname <- paste(format(Sys.time(), "%Y%m%d_%H%M%S"), "Territories", alias, param, sep="_")
pdf(fname)
layout(matrix(c(1,3,2,4), 2, 2))
...
```{r}
setwd(in.path)
fname <- paste(format(Sys.time(), "%Y%m%d_%H%M%S"), "MarkerBins", alias, param, sep="_")
pdf(fname)
layout(matrix(c(1,3,2,4), 2, 2))
